...
export const handlePrismaClientValidationError = (
  ...
  const mainMessage =
    lines.find(
      (line: string) => !line.includes('Argument') && !line.includes('→') && line.length > 10,
    ) ||
    lines[0] ||
    ...
    errorSources: [
      {
        ...
        message:
          'The database engine encountered a fatal error and crashed. Please check the Prisma logs for more details.',
      },
    ],
  };
};
